...
├── app.py                       ← app Streamlit principal (entrypoint atual)
├── dashboard_donos.py            ← alternativa/legacy de bootstrap
└── requirements.txt
```
...
- [ ] `plot_bgcolor=COR_FUNDO, paper_bgcolor=COR_FUNDO` no `update_layout` (usar `COR_FUNDO` em vez de `white`)
